...
_PROFILE_SPECS = {
    ...
    'abstractify-relationship-loss-of-feeling': {
        'opening': 'Хорошо. Если говорить в общем виде, чувство в серьёзных отношениях редко умирает по одной причине.',
        'fallback_voice': (
...
def _render_axis_followup(active_topic: str, selected_axis: str) -> tuple[str, dict]:
    relationship_axis_map = {
        'resentment': (
            'Хорошо. Если главный разрушитель здесь обида, то чувство обычно гаснет не в один день, а когда человек слишком долго ведёт внутренний счёт и перестаёт говорить правду вовремя.',
    ...
    opening, question = axis_map.get(
        selected_axis,
        (
            'Хорошо. Тогда не повторяй всю тему заново, а назови, как именно этот узел портит порядок жизни.',
...
def _render_detail_followup(active_topic: str, active_axis: str, selected_detail: str) -> tuple[str, dict]:
    ...
    opening, question = source_map.get(active_topic, {}).get(
        ...
        (
            'Хорошо. Теперь тема уже достаточно сузилась, чтобы не бегать по кругу вокруг общего симптома.',
