...
def main(theme='', pattern='', archetype=''):
    ...
    source_strength = []
    if archetype:
        row = cur.execute('SELECT r.pack_name, r.route_name, r.preferred_sources, r.preferred_quote_types FROM archetype_quote_packs a JOIN route_quote_packs r ON a.pack_id = r.id WHERE a.archetype_name = ? LIMIT 1', (archetype,)).fetchone()
        if row:
            quote_pack = {
                'pack_name': row[0],
                'route_name': row[1],
        ...
        anti_patterns = cur.execute('SELECT anti_pattern_name FROM archetype_anti_patterns WHERE archetype_name = ?', (archetype,)).fetchall()
        case_links = cur.execute('SELECT c.case_name FROM case_archetypes a JOIN cases c ON a.case_id = c.id WHERE a.archetype_name = ? LIMIT 3', (archetype,)).fetchall()
        intervention_links = cur.execute('SELECT intervention_pattern_name FROM archetype_interventions WHERE archetype_name = ? LIMIT 3', (archetype,)).fetchall()
        source_strength = cur.execute('SELECT source_name, strength FROM source_route_strength WHERE route_name = ? ORDER BY strength DESC LIMIT 3', (archetype,)).fetchall()

    if archetype in {'shame-self-contempt', 'career-vocation', 'relationship-maintenance'}:
        motif_links = cur.execute("SELECT DISTINCT s.motif_name FROM motif_cases m JOIN symbolic_motifs s ON m.motif_id = s.id JOIN case_archetypes a ON m.case_id = a.case_id WHERE a.archetype_name = ? LIMIT 3", (archetype,)).fetchall()
    ...
    if bridge:
        row = cur.execute("SELECT confidence_level, curation_level FROM confidence_tags WHERE entity_type='bridge' AND entity_id=(SELECT id FROM bridge_to_action_templates WHERE template_name=?)", (bridge[0],)).fetchone()
        if row:
            confidence_summary['bridge_confidence'] = {'confidence_level': row[0], 'curation_level': row[1]}
    if next_step:
        row = cur.execute("SELECT confidence_level, curation_level FROM confidence_tags WHERE entity_type='next_step' AND entity_id=(SELECT id FROM next_step_library WHERE step_name=?)", (next_step[0],)).fetchone()
        if row:
    ...
    if case_links:
        for case_name_tuple in case_links:
            ...
            row = cur.execute("SELECT c.id, ct.confidence_level, ct.curation_level FROM cases c LEFT JOIN confidence_tags ct ON ct.entity_type='case' AND ct.entity_id=c.id WHERE c.case_name=?", (case_name,)).fetchone()
            if row:
                best_case = {
                    'case_id': row[0],
                    'case_name': case_name,
...
if __name__ == '__main__':
    ...
    args = ap.parse_args()
    main(args.theme, args.pattern, args.archetype)
