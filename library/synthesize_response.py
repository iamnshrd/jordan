...
PROGRESS = Path('/root/.openclaw/multi-agent/agents/jordan-peterson/workspace/progress_state.json')
V3 = Path('/root/.openclaw/multi-agent/agents/jordan-peterson/library/query_v3_runtime.py')
...
def query_v3(theme='', pattern='', archetype=''):
    out = subprocess.check_output(['python3', str(V3), '--theme', theme, '--pattern', pattern, '--archetype', archetype], text=True)
    return json.loads(out)
...
def db_driven_theme_text(theme_name, selected):
    ...
    return THEME_MAP.get(theme_name, 'Нужно точнее определить, в чём ядро проблемы.')
...
def db_driven_principle_text(principle_name, selected):
    ...
    return PRINCIPLE_MAP.get(principle_name, 'Нужен принцип, который вернёт структуру и направление.')
...
def db_driven_pattern_text(pattern_name, selected):
    ...
    return PATTERN_MAP.get(pattern_name, 'Здесь есть повторяющийся разрушительный паттерн, который стоит назвать точнее.')
...
def should_suppress_progress_extra(question, practical, next_step_v3, anti_patterns, intervention_links):
    q = (question or '').lower()
    practical = (practical or '').lower()
    if next_step_v3.get('confidence_level') == 'high' and len((next_step_v3.get('step_text') or '').split()) >= 8:
        return True
    if 'abstract-inflation' in (anti_patterns or []):
        return True
    if 'narrow-burden' in (intervention_links or []):
        return True
    if any(x in q for x in ['жена', 'муж', 'отношен', 'конфликт']) and any(x in practical for x in ['разговор', 'скажи', 'обсуди']):
        return True
    if any(x in q for x in ['стыд', 'позор', 'отвращение к себе']) and any(x in practical for x in ['восстановительный шаг', 'не превращай', 'признания поступка']):
        return True
    return False
...
def compress_longer_term(longer_term, best_case, anti_patterns, v3):
    ...
    shame_case = best_case.get('case_name') == 'Shame marks exposed insufficiency before reorganization'
    shame_guard = 'identity-annihilation' in (anti_patterns or [])

    if best_case.get('case_name'):
        if shame_case and shame_guard:
            add('Ближайший аналог: Shame marks exposed insufficiency before reorganization. Смысл здесь не в самоуничтожении, а в реорганизации после честного признания.')
        else:
            case_line = f"Ближайший аналог: {best_case['case_name']}."
            if best_case.get('confidence_level') == 'high':
                case_line += ' Это высоко-уверенный ориентир, а не случайная аналогия.'
            else:
                case_line += ' Это полезная рабочая аналогия, но не окончательный якорь.'
            add(case_line)

    if shame_guard and not shame_case:
        add('Не превращай исправление в театральное самоуничтожение.')

    if v3.get('symbolic_permission') and 'dragon' in (v3.get('motif_links') or []):
        add('Здесь есть мотив встречи с неизвестным, а не только мотив поломки.')

    return ' '.join(parts).strip()


def collect_practical_extras(question, practical, next_step_v3, anti_patterns, intervention_links, motif_links, v3, progress_extra):
    extras = []
    q = (question or '').lower()

    if 'abstract-inflation' in anti_patterns and any(x in q for x in ['карьер', 'призвание', 'путь']):
        extras.append(('anti-pattern', 'Не раздувай это в метафизическую драму: выбери один рабочий вектор.'))

    if intervention_links and 'narrow-burden' in intervention_links:
        extras.append(('intervention', 'Сузь задачу до одного добровольно принятого бремени.'))

    if v3.get('symbolic_permission') and motif_links and 'burden' in motif_links:
        extras.append(('symbolic', 'Смотри на это как на вопрос о том, какое бремя ты готов нести.'))

    if progress_extra and not should_suppress_progress_extra(question, practical, next_step_v3, anti_patterns, intervention_links):
...
def collect_longer_extras(motif_links, v3):
    extras = []
    if v3.get('symbolic_permission') and motif_links and 'dragon' in motif_links:
        extras.append(('symbolic', 'Здесь есть мотив встречи с неизвестным, а не только мотив поломки.'))
    return extras


def apply_priority_pruning(practical, longer_term, practical_extras, longer_extras):
    practical_priority = {'anti-pattern': 4, 'intervention': 3, 'progress': 2, 'symbolic': 1}
    longer_priority = {'anti-pattern': 3, 'symbolic': 2}

    if practical_extras:
        practical_extras = sorted(practical_extras, key=lambda x: -practical_priority.get(x[0], 0))
        practical = append_sentence(practical, practical_extras[0][1])
    if longer_extras:
        longer_extras = sorted(longer_extras, key=lambda x: -longer_priority.get(x[0], 0))
...
def synthesize(question):
    ...
    if progress.get('progress_state') == 'stuck':
        practical = 'Следующий шаг — перестать расширять проблему и выбрать одну конкретную обязанность или один разговор, который ты откладываешь, и сделать только его.'
        longer_term = 'Сейчас тебе меньше всего нужен новый красивый анализ. Тебе нужна повторяемая дисциплина в одной точке, пока не появится реальное движение.'
        ...
        practical = 'Следующий шаг — не ломать себя об глобальные выводы, а сделать один маленький, но честный шаг без самоунижения.'
        longer_term = 'Долгосрочно тебе нужна не жестокость к себе, а более устойчивая форма честности, в которой правда не превращается в самоуничтожение.'

    bridge = v3.get('bridge') or {}
    next_step_v3 = v3.get('next_step') or {}
    if bridge.get('diagnosis_stub'):
        core_problem = bridge['diagnosis_stub']
    responsibility_avoided = db_driven_responsibility_text(selected, bridge, question)
    ...
    if bridge_conf.get('confidence_level') not in {'high'} and bridge.get('diagnosis_stub'):
        core_problem = 'Нужно ещё точнее проверить рамку, но текущая рабочая формулировка такая: ' + core_problem
    if next_step_v3.get('confidence_level') not in {'high'} and next_step_v3.get('step_text'):
        practical = 'Рабочий, но ещё не окончательно проверенный следующий шаг: ' + practical
    ...
    longer_term = compress_longer_term(longer_term, best_case, anti_patterns, v3)
